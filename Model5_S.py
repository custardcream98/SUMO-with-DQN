...
class AV100_M5_S(gym.Env):
    def __init__(self, env_config):  
        # 효과평가지표 저장 파일 초기화
        with open('AV100_M5_S_2.csv', 'w', newline='', encoding='utf-8') as csvfile:
            writer = csv.DictWriter(csvfile, fieldnames=MOE_HEADERS)
            writer.writeheader()

        # 시나리오(네트워크, 교통량) 파일 할당
        self.net_file = "intersection.av100.net.xml"
        self.route_file = "intersection.av100.rou.xml"
    ...
    def step(self, action): # 상태 리턴 -> 행동 결정 -> step: 1. 행동(신호 제어) 적용 / 2. 상태 관측 / 3. 보상 부여 / 4. 상태, 보상, 에피소드 중단 여부 리턴
        selectedAction = 2 * action
        
        # 현재 현시 정보 획득
        self.curPhase = traci.trafficlight.getPhase('gneJ00') # 0~7

        if selectedAction != self.curPhase:
            self.goto_selected_phase(selectedAction)
        else:
        ...
        if traci.simulation.getMinExpectedNumber() > 0: 
            ...
            with open('AV100_M5_S_2.csv', 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=MOE_HEADERS)
    ...
    def goto_selected_phase(self, selectedPhase):
        next_phase = []
        next_phase.append(self.curPhase + 1) # 황색 신호
        next_phase.append(selectedPhase) # 선택된 신호
        
        for phase in next_phase:
            traci.trafficlight.setPhase('gneJ00', phase)
            self.curPhaseDuration = 0
            curPhaseAll = traci.trafficlight.getAllProgramLogics('gneJ00')[0].phases[phase]
            self.sim_min_green(curPhaseAll.minDur)
        self.curPhase = selectedPhase
    ...
    def comptue_reward(self):
        ...
        for lane_ID in self.incomingLanes:
            ...
            weightedSpeed_hat += self.incomingLanes[lane_ID][7]
            weights_hat.append(self.incomingLanes[lane_ID][1])
            # queueLength_hat += self.incomingLanes[lane_ID][3]
        ...
        # type 2
        if np.sum(weights_hat) == 0:
            weightedSpeed_hat = 0
        else:
            weightedSpeed_hat = weightedSpeed_hat / np.sum(weights_hat)
        return weightedSpeed_hat
        # type 3
        # return -(queueLength_hat)

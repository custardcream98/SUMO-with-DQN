...
MOE_HEADERS = [
    ...
    'moe_queueLength',
    'moe_avgSpeed',
    'moe_totalWaitingTime_left',
    'moe_queueLength_left',
    'moe_avgSpeed_left',
]
...
class SumoEnvironment(gym.Env):
    def __init__(self, env_config):
        ...
        #     writer.writeheader()
        with open('result_moe.csv', 'w', newline='', encoding='utf-8') as csvfile:
        ...
        self.moe_avgSpeed = 0
        self.moe_totalWaitingTime_left = 0
        self.moe_queueLength_left = 0
        self.moe_avgSpeed_left = 0

        self.reward_count = 0
        traci.close()

        # 대기행렬의 길이 self.incomingLanes[lane_ID][2] -> 평균
        # 총 대기시간 self.incomingLanes[lane_ID][4] -> 누적
        # 평균 속력 self.incomingLanes[lane_ID][6] -> 평균
    ...
    def step(self, action): # 시뮬레이션의 매 스텝별 명령 메소드, 상태와 보상 리턴 후 결정된 행동이 입력 
       # 현재 현시 정보 획득
        ...
        if traci.simulation.getTime() < self.sim_max_time:
            ...
            moe_queueLength_left = self.moe_queueLength_left / self.reward_count
            moe_avgSpeed_left = self.moe_avgSpeed_left / self.reward_count
            ...
            result_moe = [{
                ...
                "moe_totalWaitingTime_left": self.moe_totalWaitingTime_left,
                "moe_queueLength_left": moe_queueLength_left,
                "moe_avgSpeed_left": moe_avgSpeed_left
            },]

            # with open('result.csv', 'a', newline='', encoding='utf-8') as csvfile:
            #     writer = csv.DictWriter(csvfile, fieldnames=HEADERS)
            #     writer.writerows(self.result_by_lanes)
            with open('result_moe.csv', 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=MOE_HEADERS)
    ...
    def compute_observation(self): # 상태 관측 메소드
        ...
        # 차로별 상태 리턴
        for lane_ID in self.incomingLanes:
            state_carCount.append(self.incomingLanes[lane_ID][0])
            state_queueLength.append(self.incomingLanes[lane_ID][2])
            state_waitingTime.append(self.incomingLanes[lane_ID][4])
            state_avgSpeed.append(self.incomingLanes[lane_ID][6])
            state_avgGapSpace.append(self.incomingLanes[lane_ID][8])
    ...
    def comptue_reward(self):
        ...
        for lane_ID in self.incomingLanes:
            if lane_ID.split('_')[1] == '0': # 직진
                waitingTime += self.incomingLanes[lane_ID][4]
                queueLength += self.incomingLanes[lane_ID][2]
                avgSpeed += self.incomingLanes[lane_ID][6]
            else:
                waitingTime_left += self.incomingLanes[lane_ID][4]
                queueLength_left += self.incomingLanes[lane_ID][2]
                avgSpeed_left += self.incomingLanes[lane_ID][6]
        
        self.moe_totalWaitingTime += waitingTime
        self.moe_queueLength += (queueLength / (len(self.incomingLanes)/2))
        self.moe_avgSpeed += (avgSpeed / (len(self.incomingLanes)/2))

        self.moe_totalWaitingTime_left += waitingTime_left
        self.moe_queueLength_left += (queueLength_left / (len(self.incomingLanes)/2))
        self.moe_avgSpeed_left += (avgSpeed_left / (len(self.incomingLanes)/2))

        return -(self.moe_queueLength + self.moe_queueLength_left)

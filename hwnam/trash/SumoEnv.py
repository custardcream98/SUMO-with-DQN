...
class SumoEnvironment(gym.Env):
    def __init__(self, env_config):
        self.sumo_mode = "sumo-gui" if env_config["is_test_run"] else "sumo"
    ...
    def reset(self): # 에피소드 초기화 메소드
        ...
        runSUMO = [self.sumo_mode, "-n",  self.net_file, '-r', self.route_file, '--random', '--quit-on-end', "--no-step-log", "--duration-log.disable", "--no-warnings", "--start"]
        traci.start(runSUMO)

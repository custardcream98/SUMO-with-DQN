...
class SumoEnvironment(gym.Env):
    def __init__(self, env_config):  
        # 효과평가지표 저장 파일 초기화
        with open('moe_model1_queue.csv', 'w', newline='', encoding='utf-8') as csvfile:
        ...
        # 행동: 모델의 출력값 (신호 제어 행동)
        ## Model1
        ### 현재 phase 유지 혹은 다음 phase로의 변경
        ### 벡터 길이: 0 또는 1 -> 2
        self.action_space = gym.spaces.Discrete(2)
        
        ## 제약조건
        ### 현시 순서 고정, 최소/최대 녹색시간, 주기 고정
        ### 마지막 phase는 잔여 시간 할당 
        self.phaseCount = 8
        self.curPhase = 0
    ...
    def step(self, action): # 상태 리턴 -> 행동 결정 -> step: 1. 행동(신호 제어) 적용 / 2. 상태 관측 / 3. 보상 부여 / 4. 상태, 보상, 에피소드 중단 여부 리턴
        ...
        if action == 1: # 다음 phase로 변경
            ...
            if (self.cycleLength - self.cycleElapsedTime) == self.remainPhasesMinDuration: # 나머지 phase들의 최소 녹색시간 조건 발동
                self.goto_next_phase_by_mindur()
        ...
        if traci.simulation.getMinExpectedNumber() > 0: 
            ...
            with open('moe_model1_queue.csv', 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=MOE_HEADERS)
    ...
    def compute_remainPhasesMinDur(self, curPhase): # 잔여 최소 녹색시간 계산
        remainMinDur = 0
    ...
    def sim_remain_phase(self): # 마지막 phase 잔여 녹색시간 할당
    ...
    def comptue_reward(self):
        ...
        for lane_ID in self.incomingLanes:
            ...
            queueLength += self.incomingLanes[lane_ID][2] + 7.5
            ...
            queueLength_hat += self.incomingLanes[lane_ID][3] + 7.5

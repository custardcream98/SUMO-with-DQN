...
class SumoEnvironment(gym.Env):
    def __init__(self, env_config):  
        # 효과평가지표 저장 파일 초기화
        with open('moe_model4_queue.csv', 'w', newline='', encoding='utf-8') as csvfile:
        ...
        self.route_file = "intersection.av80.rou.xml"
    ...
    def step(self, action): # 상태 리턴 -> 행동 결정 -> step: 1. 행동(신호 제어) 적용 / 2. 상태 관측 / 3. 보상 부여 / 4. 상태, 보상, 에피소드 중단 여부 리턴
        ...
        if traci.simulation.getMinExpectedNumber() > 0: 
            ...
            with open('moe_model4_queue.csv', 'a', newline='', encoding='utf-8') as csvfile:
                writer = csv.DictWriter(csvfile, fieldnames=MOE_HEADERS)
    ...
    def goto_next_phase_by_max(self):
        ...
        if self.curPhase == 6:
            ...
            self.curPhase += 2
        next_phase.append(self.curPhase) # 다음 신호
